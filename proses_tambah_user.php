...
if ($_POST) {
    ...
    if (empty($nama_user)) {
        ...
        echo "<script>alert('level tidak boleh kosong');location.href='tambah_user';</script>";
    } else {
        include "koneksi.php";
        $insert=mysqli_query($conn,"insert into user (id_user, nama_user, username, password, level) 
        value ('".$id_user."','".$nama_user."','".$username."','".$password."','".$level."')") or die(mysqli_error($conn));
        if ($insert) {
